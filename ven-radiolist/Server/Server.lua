...
if Config.UseRPName then
	...
	elseif GetResourceState('ven-Framework') ~= 'missing' then
		Framework = 'ven'
		Core = exports['ven-Framework']:getSharedObject()
	end
end
...
AddEventHandler("playerDropped", function()
	...
	for _, player in pairs(playersInCurrentRadioChannel) do
		--if player.Source ~= src then
			TriggerClientEvent("ven-RadioList:Client:SyncRadioChannelPlayers", player.Source, src, 0, playersInCurrentRadioChannel)
		--end
	end
...
function Connect(src, currentRadioChannel, radioChannelToJoin)
	...
	for _, player in pairs(playersInCurrentRadioChannel) do
		TriggerClientEvent("ven-RadioList:Client:SyncRadioChannelPlayers", player.Source, src, radioChannelToJoin, playersInCurrentRadioChannel)
	end
	playersInCurrentRadioChannel = {}
end
...
function Disconnect(src, currentRadioChannel) 
	local playersInCurrentRadioChannel = CreateFullRadioListOfChannel(currentRadioChannel)
	TriggerClientEvent("ven-RadioList:Client:SyncRadioChannelPlayers", src, src, 0, playersInCurrentRadioChannel)
	for _, player in pairs(playersInCurrentRadioChannel) do
		TriggerClientEvent("ven-RadioList:Client:SyncRadioChannelPlayers", player.Source, src, 0, playersInCurrentRadioChannel)
	end
	playersInCurrentRadioChannel = {}
